 ...
 * Connected to live backend API with fallback to local mock data.
 */
const SubscriptionsPage = () => {
  ...
  const fetchSubscriptions = async () => {
    ...
    try {
      ...
      setSubscriptions(Array.isArray(data) && data.length > 0 ? data : mockSubscriptionsData.subscriptions);
    } catch (err) {
      console.error('[Subscriptions] Fetch error:', err);
      setSubscriptions(mockSubscriptionsData.subscriptions);
      setError('Using local data — backend connection offline.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const summary = useMemo(() => {
    ...
    const totalMonthly = active.reduce((acc, s) => acc + (s.price || s.costUSD || s.monthlyUSD || 0), 0);
  ...
  const filteredSubscriptions = useMemo(() => {
    ...
    result.sort((a, b) => {
      const getPrice = (item) => {
        if (currency === 'INR') return item.costINR || item.priceINR || item.monthlyINR || item.price || 0;
        return item.costUSD || item.priceUSD || item.monthlyUSD || item.price || 0;
      };
